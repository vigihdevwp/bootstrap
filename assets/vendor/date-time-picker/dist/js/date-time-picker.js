...
(function (global, factory) {
    ...
    class Display {
        ...
        _iconTag(iconClass) {
            ...
            const material = classList.slice(0, 1)?.pop()
            if (material?.slice(0, 8) === 'material') {
                const icon = document.createElement('span');
                icon.classList.add(...classList.slice(0, -1));
                icon.innerText = classList.pop()
                return icon;
            }
...
(function ($) {
    ...
    const VERSION = '4.6.2'
    const DATA_KEY = 'app.datePicker'
    const EVENT_KEY = `.${DATA_KEY}`
    const JQUERY_NO_CONFLICT = $.fn[NAME]

    const SELECTOR = '.form-control.date-picker';
    ...
    window.app = window.app || {};
    window.app.DatePicker = DatePicker;
